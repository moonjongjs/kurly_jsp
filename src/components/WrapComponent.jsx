...
export default function WrapComponent() {
    ...
    React.useEffect(()=>{
        ...
        if( localStorage.getItem(signin.signinKey)!==null ){
            ...
            if( new Date() > result.expires){
                confirmModalOpen('만료일이 지났습니다. 로그아웃되었습니다.')
                setSigin({
                    ...signin,
                    user_id: '',
                    expires: ''
    ...
    const [product, setPoduct] = React.useState({
        viewKey: VIEWPRODUCTKEY,
        sign: false,
        getViewProduct : []
    });

    // 비구조화 === 구조분할할당
    const {getViewProduct, viewKey, sign} = product;
    ...
    const setViewProduct=(value)=>{
        ...
        if(localStorage.getItem(viewKey)!==null){
            arr = JSON.parse(localStorage.getItem(viewKey));
            arr = [value, ...arr]
            localStorage.setItem(viewKey, JSON.stringify(arr) );  
            setPoduct({
                ...product,
                sign: !sign,
                getViewProduct: arr
